...
def read_track_meta_csv(arguments):
    df = pandas.read_csv(arguments)
    grouped = df.groupby([LANECHANGE], sort=False)
    LC_id = [grouped.groups[1].values]
    return LC_id[0], df['class']


def read_track_csv(arguments):
    """
    This method reads the tracks file from highD HighD.
    ...
    """
    # Read the csv file, convert it into a useful HighD structure
    # df = pandas.read_csv(arguments["input_path"])
    df = pandas.read_csv(arguments)

    # Use groupby to aggregate track info. Less error prone than iterating over the HighD.
    ...
    for group_id, rows in grouped:
        if rows[X_VELOCITY].values[0] < 0:
            rows[X] = -rows[X].values - rows[WIDTH].values
            rows[X_VELOCITY] = -rows[X_VELOCITY].values
            rows[X_ACCELERATION] = -rows[X_ACCELERATION].values
        # else:
        rows[Y] = -rows[Y].values
        rows[Y_VELOCITY] = -rows[Y_VELOCITY].values
        rows[Y_ACCELERATION] = -rows[Y_ACCELERATION].values
...
def read_dataset(base_path, dataset_id):
    cache = path.join(base_path, "dataset_" + str(dataset_id) + '.pkl')
    if path.exists(cache):
        try:
            fp = open(cache, 'rb')
            track = pickle.load(fp)
            fp.close()
            print("Load dataset", dataset_id, ": done")
        except UnpicklingError:
            os.remove(cache)
            print('Removed broken cache:', cache)
    else:
        dataset_path = base_path + str("{:02d}".format(dataset_id)) + "_tracks.csv"
        track = read_track_csv(dataset_path)  # with preprocessing
        output_file = open(base_path + "dataset_" + str(dataset_id) + '.pkl', 'wb')
        pickle.dump(track, output_file)
        print("Saved dataset", dataset_id, ": done")
    return track


def find_id_in_frame(arguments, base_path, dataset_id):
    cache = path.join(base_path, "id_per_frame_dataset" + str(dataset_id) + '.pkl')
    if path.exists(cache):
        try:
            fp = open(cache, 'rb')
            tracks_id = pickle.load(fp)
            fp.close()
            print("Load id_per_frame_dataset", dataset_id, ": done")
        except UnpicklingError:
            os.remove(cache)
            print('Removed broken cache:', cache)
    else:
        ...
        tracks_id = [None] * grouped.ngroups
        current_frame = 0
        for group_id, rows in grouped:
            tracks_id[current_frame] = {FRAME: np.int64(group_id),
                                        TRACK_ID: rows.id.values}
            current_frame = current_frame + 1
        output_file = open(base_path + "id_per_frame_dataset" + str(dataset_id) + '.pkl', 'wb')
        pickle.dump(tracks_id, output_file)
        print("Saved id_per_frame_dataset", dataset_id, ": done")
    return tracks_id


def read_static_info(arguments):
    """
    This method reads the static info file from highD HighD.

    :param arguments: the parsed arguments for the program containing the input path for the static csv file.
    :return: the static dictionary - the key is the track_id and the value is the corresponding HighD for this track
    """
    # Read the csv file, convert it into a useful HighD structure
...
def read_meta_info(arguments):
    """
    This method reads the video meta file from highD HighD.
